...
void add_round_key(aesblock* S, aesblock* key){
	for(int i = 0; i < 4; ++i)
		S->rows[i] ^= key->rows[i];
}
...
void sub_bytes(aesblock *S){
	for(int i = 0; i < 16; ++i)
		S->bytes[i] = sub_box[S->bytes[i]];
}


void inv_sub_bytes(aesblock *S){
	for(int i = 0; i < 16; ++i)
		S->bytes[i] = inv_sub_box[S->bytes[i]];
}
...
uint8_t multiply_bytes(uint8_t x, uint8_t y){
    uint32_t    tmp  = x,
                mask = y,
                acc  = 0;
...
aesctx* aes_new_ctx(aesmode mode){
	int NB = 4, NK, NR;
